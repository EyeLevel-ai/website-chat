...
function resizeSections() {
  ...
  var changed = false;
  for (var i = 0; i < sections.length; i++) {
    if (sections[i].style.height !== nextHeight) {
      sections[i].style.height = nextHeight;
      changed = true;
    }

    var currentTop = parseFloat(sections[i].style.top);
    if (!isViewportNumber(currentTop)) {
      currentTop = 0;
    }

    var bounds = sections[i].getBoundingClientRect();
    var renderedTop = bounds && bounds.top;
    var correctedTop = currentTop;
    if (isViewportNumber(renderedTop)) {
      correctedTop = currentTop - renderedTop;
    }
    if (Math.abs(correctedTop) < 0.01) {
      correctedTop = 0;
    }

    var nextTop = correctedTop + 'px';
    if (sections[i].style.top !== nextTop) {
...
try {
  ...
  window.setBadge = function(n) {
    ...
    af.id = "eyBadgeCnt";
    af.innerHTML = '<iframe id="eyBadge" aria-label="notification badge" class="ey-badge-frame ey-iframe" data-hj-allow-iframe=""></iframe>';
  ...
  window.setAlert = function(txt) {
    ...
    af.id = "eyAlertCnt";
    af.innerHTML = '<iframe id="eyAlert" aria-label="notification message" class="ey-alert-frame ey-iframe" data-hj-allow-iframe="" style="background: transparent;"></iframe>';
    document.body.appendChild(af);
  ...
  window.initChatBubble = function(username, flowname, shouldOpen, video) {
    ...
    eb.innerHTML = '<iframe id="eyAppFrame" aria-label="chat launcher button" class="ey-app-container ey-iframe" data-hj-allow-iframe=""></iframe>';
  ...
  window.initChatFrame = function(username, flowname, shouldOpen, origin, attn, embed) {
    ...
    sn.id = "eySection";
    sn.innerHTML = '<iframe id="eyFrame" aria-label="chat message window" class="ey-container ey-iframe" data-hj-allow-iframe=""></iframe>';
    if (origin === 'linkedin'
      || origin === 'pdf') {

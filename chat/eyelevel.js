try {
  ...
  window.initChatBubble = function(username, flowname, shouldOpen) {
    if(window.hideChat) {
      return;
    }
  ...
  window.initChatFrame = function(username, flowname, shouldOpen, origin) {
    if(window.hideChat) {
      return;
    }
    var width = getWidth();
...
(function() {
  ...
  function toggleChat(e) {
    ...
    if (cb.classList.contains("ey-app-open")) {
      ...
      if (shouldTrack) {
        gtag('event', window.location.hostname, { event_category: 'chat_close', uid: window.eyuserid, username: window.eyusername, flowname: window.eyflowname, origin: window.eyorigin, channel: window.eychannel, shouldOpen: window.eyshouldopen });
      }
      setTimeout(function() {
      ...
      if (shouldTrack) {
        gtag('event', window.location.hostname, { event_category: 'chat_open', uid: window.eyuserid, username: window.eyusername, flowname: window.eyflowname, origin: window.eyorigin, channel: window.eychannel, shouldOpen: window.eyshouldopen });
      }
      setTimeout(function() {
        var is = document.getElementById("eyFrame");
        is = is.contentWindow || ( is.contentDocument.document || is.contentDocument);
  ...
  var eyelevel = {
    init: function(params) {
      ...
      var origin = params.origin && params.origin.toLowerCase();
      var og = getQueryVar("eyorigin");
      if (og) {
        origin = og;
      }
      if (!origin) {
      ...
      if (document && document.body) {
        ...
        if (shouldTrack) {
          gtag('event', window.location.hostname, { event_category: 'chat_load:' + version, uid: window.eyuserid, username: window.eyusername, flowname: window.eyflowname, origin: window.eyorigin, channel: window.eychannel, shouldOpen: window.eyshouldopen });
        }
        if (!window.WebSocket || !window.addEventListener) {
          if (shouldTrack) {
            gtag('event', window.location.hostname, { event_category: 'chat_hidden', uid: window.eyuserid, username: window.eyusername, flowname: window.eyflowname, origin: window.eyorigin, channel: window.eychannel, shouldOpen: window.eyshouldopen });
          }
          return;
        }
      } else {
        document.addEventListener('DOMContentLoaded', function () {
          ...
          shouldTrack = (window.location.host.indexOf('localhost') > -1 || window.location.host.indexOf('speatly') > -1 || typeof gtag === 'undefined') ? false : true;
          if (shouldTrack) {
            gtag('event', window.location.hostname, { event_category: 'chat_load', uid: window.eyuserid, username: window.eyusername, flowname: window.eyflowname, origin: window.eyorigin, channel: window.eychannel, shouldOpen: window.eyshouldopen });
          }
          if (!window.WebSocket || !window.addEventListener) {
            if (shouldTrack) {
              gtag('event', window.location.hostname, { event_category: 'chat_hidden', uid: window.eyuserid, username: window.eyusername, flowname: window.eyflowname, origin: window.eyorigin, channel: window.eychannel, shouldOpen: window.eyshouldopen });
      ...
      if (window.eyorigin === 'linkedin') {
        window.initChatStyle(window.eychannel, window.eyorigin);
        window.initChatFrame(window.eyusername, window.eyflowname, window.eyshouldopen, window.eyorigin);
      } else {
        window.addEventListener("load", function() {
          window.initChatStyle(window.eychannel, window.eyorigin);
  ...
  if (typeof gtag !== 'undefined') {
    gtag('event', window.location.hostname, { event_category: 'chat_ey_error', event_label: (e && e.stack) ? e.stack : e, uid: window.eyuserid, username: window.eyusername, flowname: window.eyflowname, origin: window.eyorigin, channel: window.eychannel, shouldOpen: window.eyshouldopen });
  }
}
